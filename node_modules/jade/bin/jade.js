...
program.on('--help', function(){
  ...
  console.log('');
  console.log('    # translate hbs the templates dir');
  console.log('    $ hbs templates');
  console.log('');
  console.log('    # create {foo,bar}.html');
  console.log('    $ hbs {foo,bar}.hbs');
  console.log('');
  console.log('    # hbs over stdio');
  console.log('    $ hbs < my.hbs > my.html');
  console.log('');
  console.log('    # hbs over stdio');
  console.log('    $ echo \'h1 hbs!\' | hbs');
  console.log('');
  console.log('    # foo, bar dirs rendering to /tmp');
  console.log('    $ hbs foo bar --out /tmp ');
  console.log('');
});
...
function stdin() {
  ...
  process.stdin.on('end', function(){
    var output;
    if (options.client) {
      output = hbs.compileClient(buf, options);
    } else {
      var fn = hbs.compile(buf, options);
...
/**
 * Process the given path, compiling the hbs files found.
...
function renderFile(path, rootPath) {
  ...
  if (stat.isFile() && re.test(path)) {
    ...
    var fn = options.client
           ? hbs.compileFileClient(path, options)
           : hbs.compileFile(path, options);
    ...
    if (program.out) {
      ...
      if (rootPath && program.hierarchy) {
        ...
        if (rootPath && !hierarchyWarned) {
          console.warn('In hbs 2.0.0 --hierarchy will become the default.');
...
function getNameFromFileName(filename) {
  var file = basename(filename, '.hbs');
