...
==================
  ...
  * Improved runtime performance of hbs's string escaping ([Andreas Lubbe](https://github.com/alubbe)) and ([@ForbesLindesay](http://www.forbeslindesay.co.uk/))
...
==================
  ...
  * Use variables instead of properties of hbs, improving performance and reliability with nested templates ([@ForbesLindesay](http://www.forbeslindesay.co.uk/))
...
===================
  ...
  * remove `hbs.version` and fix `hbs --version`
...
==================
  ...
  * Allow the compiled client to use it's own hbs util functions [3rd-Eden]
...
==================
  ...
  * Added hbs manpage (`man hbs` after installation for docs)
  * Added `-D, --no-debug` to hbs(1)
  * Added `-p, --pretty` to hbs(1)
  * Added `-c, --client` option to hbs(1)
...
==================
  ...
  * Changed: replaced internal `__` var with `__hbs` [chrisleishman]
...
==================
  ...
  * Added link in readme to hbs-mode for Emacs
...
==================
  ...
  * hbs.renderFile() is back! (for express 3.x)
...
==================
  ...
  * Fixed: include files with a .hbs extension as hbs files
...
==================
  ...
  * Fixed hbs(1) support due to `res.render()` removal
...
==================
  ...
  * Removed `hbs.render()` and `hbs.renderFile()`
...
==================
  ...
  * Added `include` support for non-hbs files
...
==================
  ...
  * Added `hbs.js` for the client-side to the repo
  * Added `hbs.min.js` for the client-side to the repo
...
==================
  ...
  * Added client-side browser support via `make hbs.js` and `make hbs.min.js`.
...
==================
  ...
  * Added `mkdirs()` to hbs(1)
  * Added hbs(1) stdio support
  * Added new features to hbs(1), `--watch`, recursive compilation etc [khingebjerg]
  * Fixed pipe-less text newlines
  * Removed hbs(1) `--pipe` flag
...
==================
  ...
  * Added; Text introduces newlines to mimic the grammar.
    ...
    hbs will now mimic the written grammar, meaning that text blocks
    using the "|" margin character will introduce a literal newline,
    where as immediate tag text (ex "a(href='#') Link") will not.

    This may not be ideal, but it makes more sense than what hbs was
    previously doing.
...
==================
  ...
  * Added support for `hbs.renderFile()` to utilize primed cache
  * Added link to [textmate bundle](http://github.com/miksago/hbs-tmbundle)
